...
|`addReadListener(object, node, CB)`| This function attaches a callback `CB` to an objects's node to listen for new values it receives and process them accordingly. <br><br>Example:<br> <pre>server.addReadListener("legoCar", "sensor", function (data) {<br>  console.log(data.value);<br>});</pre>|
|`removeReadListeners(object)`|Removes the callbacks attached to an object by addReadListener.<br><br>Example:<br> <pre>server.removeReadListeners("legoCar");</pre>|
|`addEventListener(option, callBack)`|Listens for specific events from the server. At this moment, option can be either `reset` or `shutdown`.<br><br>Example:<br> <pre>server.addEventListener('shutdown', function() {<br>  console.log('shutting down');<br>});</pre>||
|`map(value, in_min, in_max, out_min, out_max)`|Remaps the value of x from the range of `[in_min, in_max]` to the range of `[out_min, out_max]`<br><br>Example:<br> <pre>var scaledValue = server.map(data.value, 0, 1, -100, 100);</pre>| 